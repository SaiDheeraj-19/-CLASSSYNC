...
const AttendanceManager = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Controls Row */}
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-4 space-y-4">
                {/* Row 1: Selection Filters & Save Action */}
                <div className="flex flex-col xl:flex-row gap-4 items-start xl:items-center justify-between">
                    ...
                    <div className="flex flex-wrap gap-4 items-center flex-1">
                        <div>
                            ...
                            {lastSessionPreview && selectedSubject === lastSessionPreview.subject && (
                                <div className="absolute mt-1 bg-cyber-dark border border-neon-purple p-2 z-20 text-[10px] whitespace-nowrap animate-fade-in shadow-2xl">
                    ...
                    {/* Right: Save Card (Now in top row for visibility) */}
                    <div className="flex flex-col sm:flex-row items-center gap-4 bg-black/40 border border-neon-green/30 p-2 rounded w-full xl:w-auto">
                        <div className="text-xs text-gray-400 text-center sm:text-right hidden sm:block">
                            <div className="text-neon-green font-bold text-sm">{selectedSubject || 'No Subject'}</div>
                            <div className={isFutureDate() ? 'text-red-400' : 'text-neon-yellow'}>
                        ...
                        <div className="flex items-center gap-2 bg-black/50 px-2 py-1 rounded border border-white/10">
                            ...
                            />
                            <label htmlFor="updateStats" className="text-xs text-gray-400 cursor-pointer select-none whitespace-nowrap">
                                Update Stats
                            </label>
                        </div>
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-6 py-2 font-bold text-lg uppercase tracking-wider transition-all rounded ${selectedSubject && !isFutureDate()
                                ? 'bg-neon-green text-black hover:bg-white hover:scale-105 shadow-[0_0_15px_rgba(0,255,150,0.5)] animate-pulse'
                ...
                <div className="flex flex-wrap gap-2 items-center justify-between border-t border-white/5 pt-4">
                    <div className="flex flex-wrap gap-2">
                        ...
                        <div className="flex mr-4">
                            <button
                                ...
                                className="flex items-center gap-2 bg-green-500/10 border border-green-500/50 text-green-400 hover:bg-green-500 hover:text-white px-3 py-1.5 text-xs font-bold transition-colors rounded-l-md"
                            >
                                <FaCheckDouble /> PRESENT ALL
                            </button>
                            <button
                                onClick={markAllAbsent}
                                className="flex items-center gap-2 bg-red-500/10 border border-red-500/50 text-red-400 hover:bg-red-500 hover:text-white px-3 py-1.5 text-xs font-bold transition-colors rounded-r-md border-l-0"
                            >
                                <FaTimesCircle /> ABSENT ALL
                            </button>
                        </div>
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 border px-3 py-1.5 text-xs font-bold transition-colors rounded ${selectedSubject
                                ? 'bg-neon-purple/10 border-neon-purple/50 text-neon-purple hover:bg-neon-purple hover:text-white'
                                : 'bg-gray-800 border-gray-700 text-gray-500 cursor-not-allowed'}`}
                        >
                            <FaCheckDouble /> APPLY LAST
                        </button>

                        <div className="relative group">
                            <select
                                className="bg-black/30 border border-gray-700 text-gray-400 hover:text-white px-3 py-1.5 text-xs font-bold transition-colors cursor-pointer appearance-none pr-6 focus:outline-none focus:border-neon-blue rounded"
                                onChange={(e) => {
                            ...
                            <FaSortAmountUp className="absolute right-2 top-2 text-gray-500 pointer-events-none text-xs" />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        <button
                            ...
                            className="bg-black/30 border border-white/20 text-gray-400 hover:text-white px-3 py-1.5 text-xs font-bold transition-colors rounded"
                        >
                        ...
                        <button
                            ...
                            className="bg-black/30 border border-white/20 text-neon-purple hover:text-white px-3 py-1.5 text-xs font-bold transition-colors rounded"
                        >
                            {isConfigMode ? 'Done' : 'Manage Subjects'}
                        </button>

                        <button
                            onClick={downloadAbsentees}
                            className="bg-neon-blue/10 border border-neon-blue/50 text-neon-blue hover:bg-neon-blue hover:text-black px-3 py-1.5 text-xs font-bold transition-colors rounded flex items-center gap-2"
                        >
                            <FaDownload /> CSV
                        </button>
                    </div>
